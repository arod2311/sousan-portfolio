...
// Additional service slip CC recipients
const SERVICE_SLIP_ALWAYS_CC = (process.env.SERVICE_SLIP_ALWAYS_CC || '')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
const SERVICE_SLIP_ALWAYS_CC_NAMES = (process.env.SERVICE_SLIP_ALWAYS_CC_NAMES || '')
  .split(',')
  .map(s => s.trim());
...
async function sendServiceSlipWithTemplate(payload) {
  ...
  if (SERVICE_SLIP_ALWAYS_CC.length) {
    const extraCarbonCopies = [];
    const seenEmails = new Set();
    SERVICE_SLIP_ALWAYS_CC.forEach((email, idx) => {
      const lower = email.toLowerCase();
      if (seenEmails.has(lower)) return;
      seenEmails.add(lower);
      const nameOverride = SERVICE_SLIP_ALWAYS_CC_NAMES[idx] || '';
      const inferredName = nameOverride || (email.includes('@') ? email.split('@')[0] : email);
      extraCarbonCopies.push(docusign.CarbonCopy.constructFromObject({
        email,
        name: inferredName,
        routingOrder: '1',
        recipientId: String(9000 + idx)
      }));
    });
    if (extraCarbonCopies.length) {
      if (!envDef.recipients) envDef.recipients = new docusign.Recipients();
      const existing = Array.isArray(envDef.recipients.carbonCopies) ? envDef.recipients.carbonCopies : [];
      envDef.recipients.carbonCopies = existing.concat(extraCarbonCopies);
    }
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (isServiceSlip && completed) {
      ...
      // FIX: log the welcome gating decision so it’s obvious in logs

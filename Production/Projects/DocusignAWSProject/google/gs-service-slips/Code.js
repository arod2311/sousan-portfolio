...
const BILLING_APPROVAL_NAME_HDR = "BillingApprovalName";
const BILLING_APPROVAL_EMAIL_HDR = "BillingApprovalEmail";
...
const REQUIRED_FIELDS_FOR_SLIP = [
  ...
  DRIVER_NAME_HDR,
  DRIVER_EMAIL_HDR,
  BILLING_APPROVAL_EMAIL_HDR,
  BILLING_APPROVAL_NAME_HDR,
];
...
const PROTECT_HEADERS = ["SlipLink", "DocS3Key", "CreatedAt", "UpdatedAt", "ContainerID", "ContainerID2", "ContainerID3"];

const SERVICE_ACCOUNT_EMAIL = (
  PropertiesService.getScriptProperties().getProperty("SERVICE_ACCOUNT_EMAIL") || ""
).trim();
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem("Grant SA to protections (repair)", "grantServiceAccountToProtections")
...
function setupLookupsAndValidation() {
  ...
  if (!sheet || !look) {
    SpreadsheetApp.getUi().alert(`Missing required sheets. Ensure "${SHEET_NAME}" and "${LOOKUPS_SHEET}" exist.`);
    return;
  }
  ...
  const approverNameRange = look.getRange(2, 5, Math.max(1, lastLookRow - 1), 1);
  const approverEmailRange = look.getRange(2, 6, Math.max(1, lastLookRow - 1), 1);
  const billingNameRange = look.getRange(2, 7, Math.max(1, lastLookRow - 1), 1);
  const billingEmailRange = look.getRange(2, 8, Math.max(1, lastLookRow - 1), 1);

  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].map((h) => (h || "").toString().trim());
  const hdrMap = {};
  headers.forEach((h, i) => { if (h) hdrMap[h] = i + 1; });
  ...
  setDropdown(BILLING_APPROVAL_NAME_HDR, billingNameRange);
  setDropdown(BILLING_APPROVAL_EMAIL_HDR, billingEmailRange);
  ...
  try { look.hideSheet(); } catch (e) {}
  SpreadsheetApp.getUi().alert("Setup complete: dropdowns + Initiate checkbox applied, and columns protected.");
}

function installOnEditTrigger() {
  const project = ScriptApp.getProjectTriggers();
  project.forEach((t) => { if (t.getHandlerFunction() === "onEditHandler") ScriptApp.deleteTrigger(t); });
...
function protectOutputColumns(sheet) {
  ...
  ["SlipLink", "DocS3Key", "CreatedAt", "UpdatedAt"].forEach((h) => {
    ...
    editorsToAdd.forEach((email) => {
      if (email && current.indexOf(email) === -1) { try { prot.addEditor(email); } catch (e) {} }
    });
  ...
  if (look) {
    ...
    editorsToAdd.forEach((email) => { if (email && existing.indexOf(email) === -1) { try { prot.addEditor(email); } catch (e) {} } });
    if (prot.canDomainEdit()) prot.setDomainEdit(false);
  }
}
...
function grantServiceAccountToProtections() {
  if (!SERVICE_ACCOUNT_EMAIL) {
    SpreadsheetApp.getUi().alert("Set SERVICE_ACCOUNT_EMAIL in Script Properties first.");
    return;
  }
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAME);
  ...
  sheet.getProtections(SpreadsheetApp.ProtectionType.RANGE).forEach((p) => {
    ...
    if (d.indexOf("ServiceSlip_protect_") === 0) {
      try { p.addEditor(SERVICE_ACCOUNT_EMAIL); n++; } catch (e) {}
    }
  });
  const look = ss.getSheetByName(LOOKUPS_SHEET);
  if (look) {
    look.getProtections(SpreadsheetApp.ProtectionType.SHEET).forEach((p) => { try { p.addEditor(SERVICE_ACCOUNT_EMAIL); } catch (e) {} });
    look.getProtections(SpreadsheetApp.ProtectionType.RANGE).forEach((p) => { try { p.addEditor(SERVICE_ACCOUNT_EMAIL); } catch (e) {} });
  }
  SpreadsheetApp.getUi().alert(`Service account added to ${n} protected ranges.`);
}

/****************  EDIT FLOW  ****************/
/** FIX: simple trigger that always forwards to the handler */
function onEdit(e) {
  try {
    // Simple onEdit trigger lacks the scopes for UrlFetch; bail and let the installable trigger run.
    if (!e || e.authMode === ScriptApp.AuthMode.LIMITED) return;
    onEditHandler(e);
  } catch (err) {
    /* swallow to avoid toast loops */
  }
}
...
function onEditHandler(e) {
  try {
    ...
    // Row-level debounce only for InitiateSlip checkbox (allows normal edits)
    const row = e.range.getRow();
    const _hdrTmp = getHeaderMap(sheet);
    const _editedColTmp = e.range.getColumn();
    const _chkColTmp = findInitiateCol(_hdrTmp);
    if (_chkColTmp && _editedColTmp === _chkColTmp) {
    ...
    if (!docLock.tryLock(5000)) return;

    try { autoFillDriverPairOnEdit(e); }     catch (err) { console.error("autoFillDriverPairOnEdit err", err); }
    try { autoFillApproverPairOnEdit(e); }   catch (err) { console.error("autoFillApproverPairOnEdit err", err); }
    try { autoFillBillingPairOnEdit(e); }    catch (err) { console.error("autoFillBillingPairOnEdit err", err); }
    try { autoSetReadyStatusOnEdit(e); }     catch (err) { console.error("autoSetReadyStatusOnEdit err", err); }
    ...
    if (statusCol && editedCol === statusCol) {
      const s = (sheet.getRange(editedRow, statusCol).getValue() || "").toString();
    ...
    const isChecked =
      e.value === "TRUE" || e.value === true || String(e.value).toLowerCase() === "true";
    if (!isChecked) return;

    if (hdr["Status"]) {
      const cur = (sheet.getRange(editedRow, hdr["Status"]).getValue() || "").toString();
      if (!isReadyStatus(cur)) {
        SpreadsheetApp.getActive().toast("Not ready to initiate. Status: " + cur, "ServiceSlip", 6);
    ...
    if (missing.length > 0) {
      SpreadsheetApp.getActive().toast("Missing required: " + missing.join(", "), "ServiceSlip", 8);
      sheet.getRange(editedRow, checkboxCol).setValue(false);
      return;
    }

    const payload = buildPayloadFromRow(sheet, hdr, editedRow);
    setCellIfHeaderExists(sheet, hdr, editedRow, "Status", "Service Slip Initiated");
    setCellIfHeaderExists(sheet, hdr, editedRow, "UpdatedAt", new Date().toISOString());
    sheet.getRange(editedRow, checkboxCol).setValue(false);

    const resp = callBackendToCreateServiceSlip(payload);
    if (resp && resp.success === true) {
      const n = resp.createdCount || (resp.serviceSlipEnvelopeIds ? resp.serviceSlipEnvelopeIds.length : 1);
      SpreadsheetApp.getActive().toast(
        "Service Slip(s) initiated: " + n + (resp.serviceSlipEnvelopeId ? " (first: " + resp.serviceSlipEnvelopeId + ")" : ""),
        "ServiceSlip",
        6
      );
    } else {
      setCellIfHeaderExists(sheet, hdr, editedRow, "Status", "Ready for Service Slip");
      SpreadsheetApp.getActive().toast("Failed to send Service Slip" + (resp && resp.error ? ": " + resp.error : ""), "ServiceSlip", 8);
    }
  } catch (err) {
    console.error("onEditHandler error", err);
    try { if (e && e.range) e.range.setValue(false); } catch (ignore) {}
    SpreadsheetApp.getActive().toast("Error: " + (err && err.toString ? err.toString() : JSON.stringify(err)), "ServiceSlip", 8);
  } finally {
    try { LockService.getDocumentLock().releaseLock(); } catch (_) {}
  }
}
...
function autoSetReadyStatusOnEdit(e) {
  ...
  for (const fld of REQUIRED_FIELDS_FOR_SLIP) {
    const c = hdr[fld];
    if (!c) { allGood = false; break; }
    const v = sheet.getRange(r, c).getValue();
    if (v === "" || v === null) { allGood = false; break; }
  }
  if (allGood) {
    const prev = sheet.getRange(r, statusCol).getValue();
    if (!prev || prev === "" || prev === "Pending Slip" || prev === "Pending Assignment") {
      sheet.getRange(r, statusCol).setValue("Ready for Service Slip");
      if (hdr["UpdatedAt"]) sheet.getRange(r, hdr["UpdatedAt"]).setValue(new Date().toISOString());
    }
  }
}
...
function autoFillDriverPairOnEdit(e) {
  ...
  const editedCol = e.range.getColumn();
  const nameCol = hdr[DRIVER_NAME_HDR], emailCol = hdr[DRIVER_EMAIL_HDR];
  ...
  if (!look) return;
  const vals = look.getRange(2, 3, Math.max(1, look.getLastRow() - 1), 4).getValues();
  ...
  if (editedCol === nameCol) {
    const name = (sheet.getRange(row, nameCol).getValue() || "").toString().trim().toLowerCase();
    const email = nameToEmail.get(name) || "";
    if (emailCol) sheet.getRange(row, emailCol).setValue(email);
  } else if (editedCol === emailCol) {
    const email = (sheet.getRange(row, emailCol).getValue() || "").toString().trim().toLowerCase();
...
function autoFillApproverPairOnEdit(e) {
  ...
  const editedCol = e.range.getColumn();
  const nameCol = hdr[APPROVER_NAME_HDR], emailCol = hdr[APPROVER_EMAIL_HDR];
  if (!nameCol && !emailCol) return;
  if (editedCol !== nameCol && editedCol !== emailCol) return;

  const look = SpreadsheetApp.getActive().getSheetByName(LOOKUPS_SHEET);
  if (!look) return;
  const vals = look.getRange(2, 5, Math.max(1, look.getLastRow() - 1), 2).getValues();
...
function autoFillBillingPairOnEdit(e) {
  const sheet = e.range.getSheet();
  const hdr = getHeaderMap(sheet);
  const row = e.range.getRow();
  if (row === 1) return;
  ...
  const editedCol = e.range.getColumn();
  const nameCol = hdr[BILLING_APPROVAL_NAME_HDR], emailCol = hdr[BILLING_APPROVAL_EMAIL_HDR];
  if (!nameCol && !emailCol) return;
  if (editedCol !== nameCol && editedCol !== emailCol) return;

  const look = SpreadsheetApp.getActive().getSheetByName(LOOKUPS_SHEET);
  if (!look) return;
  const vals = look.getRange(2, 7, Math.max(1, look.getLastRow() - 1), 2).getValues();
...
function callBackendToCreateServiceSlip(payload) {
  ...
  if (!API_URL || !API_KEY) {
    return { success: false, error: "Missing API_URL or API_KEY in Script Properties" };
  }
  ...
  if (API_SEC) {
    const rawSig = Utilities.computeHmacSignature(Utilities.MacAlgorithm.HMAC_SHA_256, timestamp + "." + bodyStr, API_SEC);
    signature = rawSig.map((b) => (b < 0 ? b + 256 : b).toString(16).padStart(2, "0")).join("");
  }
...
/****************  ARCHIVE HELPERS  ****************/
function syncCompletedHeaderRow(sheet, headers) {
  if (!sheet || !headers || headers.length === 0) return;
  const headerLength = headers.length;
  const existingCols = Math.max(sheet.getLastColumn(), headerLength, 1);
  let existing = [];
  try { existing = sheet.getRange(1, 1, 1, existingCols).getValues()[0] || []; } catch (err) { existing = []; }
  let needsUpdate = sheet.getLastRow() === 0;
  if (!needsUpdate) {
    if (existing.length < headerLength) {
      needsUpdate = true;
    } else {
  ...
  if (needsUpdate) { sheet.getRange(1, 1, 1, headerLength).setValues([headers]); }
}

function maybeArchiveRow(sheet, hdrMap, row) {
  const ss = sheet.getParent();
  const archive = ss.getSheetByName(COMPLETED_SHEET) || ss.insertSheet(COMPLETED_SHEET);
  const headers = Object.keys(hdrMap).sort((a, b) => hdrMap[a] - hdrMap[b]);

  syncCompletedHeaderRow(archive, headers);
...
function validateRequiredFields(sheet, headerMap, row) {
  ...
  REQUIRED_FIELDS_FOR_SLIP.forEach((hdr) => {
    ...
    if (!col) { missing.push(hdr + " (column not found)"); return; }
...
function menuValidateSelectedRow() {
  ...
  if (sheet.getName() !== SHEET_NAME) {
    SpreadsheetApp.getUi().alert('Switch to the "' + SHEET_NAME + '" sheet first.');
    return;
  }
  const hdr = getHeaderMap(sheet);
  ...
  if (missing.length > 0) SpreadsheetApp.getUi().alert("Missing fields:\n" + missing.join("\n"));
...
function menuInitiateSelectedRow() {
  ...
  const curStatus = hdr["Status"] ? sheet.getRange(row, hdr["Status"]).getValue() : "";
  if (curStatus !== "Ready for Service Slip") {
    SpreadsheetApp.getUi().alert('Status must be "Ready for Service Slip" to initiate from the menu. Current: ' + curStatus);
    return;
  }

  const missing = validateRequiredFields(sheet, hdr, row);
  if (missing.length > 0) {
    SpreadsheetApp.getUi().alert("Missing required fields:\n" + missing.join("\n"));
    return;
  }
  ...
  if (resp && resp.success === true) {
    ...
    SpreadsheetApp.getUi().alert("Failed to initiate Service Slip: " + (resp && resp.error ? resp.error : "unknown"));
  }
}
